...
def serve_media_file(request, file_path):
    """
    Serves media files. Automatically handles Local vs S3.
    """
    try:
        # Django's default_storage knows if it's S3 or Local
        storage = settings.DEFAULT_FILE_STORAGE
        
        if 's3' in str(storage).lower():
            # For S3, we still need to verify the file exists
            # (Note: S3 storage checks existence differently)
            pass
            
        # This URL generation works for BOTH Local and S3
        # - Local: Returns /media/path/to/file.mp4
        # - S3: Returns https://...supabase.co/.../file.mp4
        file_url = media_file.file.url 
        
        # If using Local Storage, build the absolute URL for the React frontend
